 ...
 * multimodal content block, and the few response shapes the OpenAI endpoint
...
function writeReportFile(date: string, text: string): string {
  ...
  const header = [
    `# 蝶刺 分析報告 — ${date}`,
    "",
    `> 由 ${OPENAI_MODEL} 生成 · ${new Date().toISOString()} · host=${hostname()}`,
    "",
    "---",
    "",
  ].join("\n");
...
function commitReportFile(file: string, date: string): { ok: true; sha: string } | { ok: false; error: string } {
  ...
  const commit = safeExec(
    ...
    [
      ...
      "-m",
      `Generated by ${OPENAI_MODEL} via /weight → 產生今日報告.`,
      "--",
      file,
    ],
    root,
